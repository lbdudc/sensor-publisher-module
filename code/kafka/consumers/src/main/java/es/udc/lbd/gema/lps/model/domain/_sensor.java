...
public class /*%= normalize(context.id, true) %*/ {
    ...
    public String toSQL() {
      ...
      String res = "(";
      res += "nextval('t_/*%= normalize(context.id, false) %*/_id_seq')" + ",";
      /*% context.measureData.forEach(function(prop) { %*/
      /*%
        var normalizedProp = prop.name.toLowerCase();
      %*/
      res += this./*%= normalize(normalizedProp) %*/ + ", ";
      /*% }); %*/

      /*% if (entity) { %*/
      res += this./*%= normalize(context.entity, false) %*/Id + ", ";
      /*% } %*/

      res += "to_timestamp('" + this.date + "', 'YYYY-MM-DD HH24:MI:SS'), ";
      ...
      /*%
        ...
        dimensions.forEach(function(dim) {
      %*/
        res += this./*%= normalize(dim, false) %*/Id + ", ";
      /*%
        });
      %*/
