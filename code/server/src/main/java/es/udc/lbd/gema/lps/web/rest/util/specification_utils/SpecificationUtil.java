...
import es.udc.lbd.gema.lps.web.rest.util.DateTimeValidator;
import es.udc.lbd.gema.lps.web.rest.util.DateTimeValidatorImpl;
        ...
        if (parts.length == 2) {
          ...
          if (excluding) {
            genericSpecification.add(
              new SearchCriteria(parts[0], value, op, LogicalOperation.OR));
          } else {
            genericSpecification.add(
              new SearchCriteria(
                parts[0], value, op, LogicalOperation.AND));
          }
        }
      }
    }
